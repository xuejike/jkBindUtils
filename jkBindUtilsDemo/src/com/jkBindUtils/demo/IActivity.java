...
import com.jkBindUtils.demo.vo.IdBook;
...
public class IActivity extends Activity {
    ...
    // 布局文件与Java bean 通过id进行绑定，
    private ViewIdBindAdapter getViewIdBindAdapter(){
        List list=new ArrayList();
        for (int i=0;i<30;i++){
            list.add(new IdBook("图书标题"+i,drawables.get(i%drawables.size())));
        }

        ViewIdBindAdapter adapter =new ViewIdBindAdapter(this,R.layout.book_item,list);
        return adapter;
    }
}